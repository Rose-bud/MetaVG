...
def train_one_epoch_meta(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    img1, mask1, target1 = img, mask, target
    img3, mask3, target3 = img, mask, target
    iteration = 0

    while img is not None:
        inner_opti = optim.SGD(model.parameters(), lr=inner_lr)
        weight0 = model.state_dict().copy()
        ...
        for item in fast_weights.values():
            item.requires_grad = True

        gradients = torch.autograd.grad(losses, fast_weights.values(), create_graph=True, allow_unused=True)  # add
        ...
        if img == None:
            img3, mask3, target3 = img, mask, target
            img, mask, target = img1, mask1, target1
        ...
        gradients = torch.autograd.grad(losses, model.parameters(),create_graph=True, allow_unused=True)#add

        model.load_state_dict(weight0)
        for i, params in enumerate(model.parameters()):
            params.grad = gradients[i]
...
def train_one_epoch_w_accum(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    while img is not None:
        ...
        for i in range(2):
            ...
            outputs,_,_ = model(b_img, b_mask, b_word_id, b_word_mask)
...
def evaluate(model, criterion, postprocessor, data_loader, device, save_path=''):
    ...
    for iteration, (img, mask, target) in enumerate(tqdm(prefetcher)):
        ...
        outputs,_ ,_  = model(img, mask, word_id, word_mask)
